...
var kansas = ee.FeatureCollection("TIGER/2018/States").filter(ee.Filter.eq('NAME', 'Kansas'));
...
var grid = ee.FeatureCollection(
  aoi.coveringGrid(proj).map(function(f) {
    return f.set('grid_id', f.id());
  })
);

// Определение самого частой культуры в клетке и фильтрация клеток разбиения

var extractStats = function(feature) {
    var geom = feature.geometry();
    var cropHistogram = cropDataset.reduceRegion({
    ...
    var cropDict = ee.Dictionary(cropHistogram);
    var counts = ee.List(cropDict.values()).map(function(v) { return ee.Number(v); });
    var total = ee.Number(counts.reduce(ee.Reducer.sum()));
    var maxCount = ee.Number(counts.reduce(ee.Reducer.max()));

    var cropPercentage = ee.Algorithms.If(
        total.gt(0),
        maxCount.divide(total).multiply(100),
        0
    );

    var keys = ee.List(cropDict.keys());
    var maxIndex = counts.indexOf(maxCount);
    var cropCode = ee.Number.parse(keys.get(maxIndex));

    return ee.Algorithms.If(
        cropCode.lte(60).or(cropCode.gte(220)).or(cropCode.gte(66).and(cropCode.lte(77))), // фильтруем не сельскохозяйственные земли
        feature.set({
            'crop_mode': cropCode,
...
var sampledData = grid.map(extractStats, true);
var filteredData = sampledData
  .filter(ee.Filter.notNull(['crop_percentage']))
  .filter(ee.Filter.gte('crop_percentage', 80));

var grid = filteredData.select('crop_mode');

// Интересующая дата наблюдений
 ...
 */
var computeBaseFlowIndex = function(precip) {
  var filteredPrecip = precip.filterDate(START_DATE, END_DATE);
...
var BFI = computeBaseFlowIndex(era.select('total_precipitation')).rename('bfi');
...
var cellStats = featureStack.reduceRegions({
  collection: grid, 
  reducer: ee.Reducer.mean(),
  scale: cellSize,
  crs: proj,
...
var outputTable = cellStats.map(function(feature) {
  return ee.Feature(null, feature.toDictionary());
});

Export.table.toDrive({
  collection: outputTable,
  description: 'Kansas_2018_9',
  fileFormat: 'CSV'
});